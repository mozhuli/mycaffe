...
template <typename Dtype>
LayerParameter SPPLayer<Dtype>::GetSpecPoolingParam(const int pyramid_level,
      const int bottom_h, const int bottom_w, const SPPParameter spp_param) {
  LayerParameter pooling_param;
  int num_bins = num_bins_vec_[pyramid_level];
  ...
  int pad_w = (remainder_w + 1) / 2;
  /*
  LOG(INFO) << "num_bins: " << num_bins << std::endl;
  LOG(INFO) << "bottom_h: " << bottom_h << std::endl;
  LOG(INFO) << "bottom_w: " << bottom_w << std::endl;
  LOG(INFO) << "kernel_h: " << kernel_h << std::endl; 
  LOG(INFO) << "kernel_w: " << kernel_w << std::endl;
  LOG(INFO) << "pad_h: " << pad_h << std::endl;
  LOG(INFO) << "pad_w: " << pad_w << std::endl;
  */
  pooling_param.mutable_pooling_param()->set_pad_h(pad_h);
  pooling_param.mutable_pooling_param()->set_pad_w(pad_w);
  pooling_param.mutable_pooling_param()->set_kernel_h(kernel_h);
  ...
  //LOG(INFO) << "pad_h in pooling_param: " << pooling_param.mutable_pooling_param()->pad_h() << std::endl;
  //LOG(INFO) << "pad_w in pooling_param: " << pooling_param.mutable_pooling_param()->pad_w() << std::endl;
...
void SPPLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // if using num_bins, its size has to be equal to pyramid_height
  int num_bins_size = spp_param.num_bins_size();
  if(num_bins_size > 0) {
    CHECK_EQ(num_bins_size, pyramid_height_) << "Pyramid_height and num_bins_size should be equal.";
    for(int i = 0; i< pyramid_height_; i++) {
      num_bins_vec_.push_back(spp_param.num_bins(i));
    }
  }
  ...
  for (int i = 0; i < pyramid_height_; i++) {
    ...
    */
    LayerParameter pooling_param = GetSpecPoolingParam(
        i, bottom_h_, bottom_w_, spp_param);
